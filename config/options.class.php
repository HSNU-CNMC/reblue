...
class Options {
	...
	//INITIAL INSTALL OR UPDATING
		function update_or_install()
		{
			...
			if( !get_option('rb_options') && !get_option('rb_options_redundant') )
			{ 
	...
	// GET OPTION FUNCTIONS 
		...
		{
			if(!get_option('rb_options')){ 
				$saved_options = maybe_unserialize(get_option('rb_options_redundant'));
				update_option('rb_options',get_option('rb_options_redundant'));
			}else{ $saved_options = maybe_unserialize(get_option('rb_options'));	}
...
function pagelines($option, $post_id = ''){
	...
	if($post_id && get_post_meta($post_id, $option, true) && !is_home()){
		...
		global $pagelines; 
		if ( isset($pagelines->$option) && $pagelines->$option )
		{
